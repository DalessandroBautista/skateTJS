...
export class CameraSystem {
   ...
   */
  constructor({ camera, target, velocity }) {
    this.camera = camera;
    this.target = target;
  ...
  update(dt) {
    ...
    // Exponential smoothing: frame-rate independent (equivalente a lerp(k*dt) pero correcto)
    const posFactor  = 1 - Math.exp(-this.positionSmoothing * dt);
    ...
    const isGrounded = Math.abs(vel.y) < 2.5 && playerPos.y < 30;
